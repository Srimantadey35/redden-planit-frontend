...
const WeddingPlanner = ({ prefilledValues = {}, defaultValues = {}, onDataChange }) => {
  const [form, setForm] = useState({
    commercialType: prefilledValues.commercialType || defaultValues.commercialType || "",
    styleUSP: prefilledValues.styleUSP || defaultValues.styleUSP || "",
    decorPolicy: prefilledValues.decorPolicy || defaultValues.decorPolicy || "",
    plannedCities: prefilledValues.plannedCities || defaultValues.plannedCities || "",
    userCancelPolicy: prefilledValues.userCancelPolicy || defaultValues.userCancelPolicy || "",
    selfCancelPolicy: prefilledValues.selfCancelPolicy || defaultValues.selfCancelPolicy || "",
    servicesProvided: prefilledValues.servicesProvided || defaultValues.servicesProvided || "",
    businessYear: prefilledValues.businessYear || defaultValues.businessYear || "",
    celebrityWedding: prefilledValues.celebrityWedding || defaultValues.celebrityWedding || "",
    cancelTerms: prefilledValues.cancelTerms || defaultValues.cancelTerms || "",
    prominentCelebs: prefilledValues.prominentCelebs || defaultValues.prominentCelebs || "",
    advanceBookingWeeks: prefilledValues.advanceBookingWeeks || defaultValues.advanceBookingWeeks || "",
    startingPrice: prefilledValues.startingPrice || defaultValues.startingPrice || "",
    averagePrice: prefilledValues.averagePrice || defaultValues.averagePrice || "",
    destinationChange: prefilledValues.destinationChange || defaultValues.destinationChange || "",
    includedServices: prefilledValues.includedServices || defaultValues.includedServices || [],
  });
  ...
  const destinationOptions = ['Yes', 'No'];
  ...
  const toggleService = (service) => {
    const updatedServices = form.includedServices.includes(service)
      ? form.includedServices.filter(s => s !== service)
      : [...form.includedServices, service];
    handleChange("includedServices", updatedServices);
  };
  ...
  const countFilledFields = (formData) => {
    ...
    // Text/select fields
    const textFields = [
      'commercialType', 'styleUSP', 'decorPolicy', 'plannedCities',
      'userCancelPolicy', 'selfCancelPolicy', 'servicesProvided', 'businessYear',
      'celebrityWedding', 'cancelTerms', 'prominentCelebs', 'advanceBookingWeeks',
      'startingPrice', 'averagePrice', 'destinationChange'
    ];
    ...
    // Array fields
    if (formData.includedServices && formData.includedServices.length > 0) count++;
    
    return count;
  };

  const getTotalFields = () => {
    return 16; // Total number of form fields
  };
  ...
  return (
    <>
      {renderRadioGroup("commercialType", "What kind of commercials do you work on?", commercialOptions, "commercialOption")}
      
      {renderTextarea("styleUSP", "What is your style USP?")}
      
      {renderRadioGroup("decorPolicy", "What is your policy on decor?", decorOptions, "decorOption")}
      
      {renderTextarea("plannedCities", "Which are some key cities you have planned weddings in previously?")}
      
      {renderRadioGroup("userCancelPolicy", "Please describe your cancellation policy (if a user initiates cancellation)", cancelUserOptions, "userCancelOption")}
      
      {renderRadioGroup("selfCancelPolicy", "Please describe your cancellation policy (if you initiate a cancellation)", cancelSelfOptions, "selfCancelOption")}
      
      {renderTextarea("servicesProvided", "What services are provided by you?")}
      
      {renderInput("businessYear", "In which year did you start your business?")}
      
      {renderInput("celebrityWedding", "Have you planned any celebrity weddings? Please mention the name of the celebrity.")}
      
      {renderTextarea("cancelTerms", "What are the terms & conditions of your cancellation policy?")}
      
      {renderTextarea("prominentCelebs", "Would you like to mention a few names of any prominent people/celebrities whose wedding you have planned?")}
      
      {renderInput("advanceBookingWeeks", "How many weeks in advance should a booking be made?")}
      
      {renderInput("startingPrice", "What is your starting price for planning a 3 day wedding?")}
      
      {renderInput("averagePrice", "What is your average wedding planning package cost?")}
      
      {renderRadioGroup("destinationChange", "Does your planning fee change for destination weddings?", destinationOptions, "destinationOption")}
      
      {renderCheckboxGroup("includedServices", "Which of the following services are included in your planning fee?", serviceOptions, toggleService)}
