...
const Page = () => {
  ...
  const uploadChangedPassword = async (e) => {
    e.preventDefault()
    const { currentPassword, newPassword, confirmNewPassword } = formik.values
    const passwordFields = { currentPassword, newPassword, confirmNewPassword }
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL_SYSTEM}/change-password`, passwordFields,
        {
          withCredentials: true,
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const updateUserDetails = async () => {
    const { facebookUrl, instagramUrl, twitterUrl } = formik.values
    const allSocialLinks = { facebookUrl, instagramUrl, twitterUrl }

    try {
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL_SYSTEM}/vendors/update-vendor-details`, { firstName, lastName, ...allSocialLinks }, {
        withCredentials: true,
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  //   setSocialLinks(prev => ({
  //     ...prev,
  //     [`${platform.toLowerCase()}Url`]: value
  //   }));
  //   setHasChanged(true)
  // };
  return (
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    {socialMedia.map((item, index) => (
                      <div key={index} className="flex items-center space-x-2.5">
                        ...
                        <input
                          className="h-[42px] 3xl:h-[53px] rounded-[8px] outline-none bg-white px-[22px] placeholder:text-[#525252] 3xl:placeholder:text-[16px] 3xl:text-[16px] placeholder:text-[14px] text-[14px] font-medium text-black w-full"
                          placeholder={`${item.socialMediaName} link`}
                          type="text"
                          value={formik.values[`${item.socialMediaName.toLowerCase()}Url`] || ""}
                          onChange={(e) => {
                            formik.handleChange(e);
                            setHasChanged(true);
                          }}
                          onBlur={formik.handleBlur}
                          name={`${item.socialMediaName.toLowerCase()}Url`}
                          id={`${item.socialMediaName}`}
                          disabled={!isEditing}
                          readOnly={!isEditing}
                        />
                        {formik.touched[`${item.socialMediaName.toLowerCase()}Url`] &&
                          formik.errors[`${item.socialMediaName.toLowerCase()}Url`] && (
                            <p className="text-red-600 text-sm mt-1 ml-[36px]">
                              {formik.errors[`${item.socialMediaName.toLowerCase()}Url`]}
                            </p>
                          )}
                      </div>
            ...
            {showPasswordModal && (
              <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/40 backdrop-blur-sm md:w-[calc(100%-200px)] md:ml-[200px] smd:w-[calc(100%-245px)] 3xl:w-[calc(100%-280px)] smd:ml-[245px] 3xl:ml-[280px]">
                <div className="bg-[#F2F2F2] rounded-[10px] p-6 w-full max-w-[400px] shadow-lg relative">
                  ...
                  <form
                    onSubmit={uploadChangedPassword}
                  >
                    <div className="flex flex-col mb-3 relative">
                      ...
                      />
                      <span
                        className={`absolute right-3 top-10 ${formik.values.currentPassword.trim() ? 'cursor-pointer' : 'pointer-events-none opacity-50'
                          }`}
                        onClick={() => {
                          if (!formik.values.currentPassword.trim()) return;
                    ...
                    <div className="flex flex-col mb-3 relative">
                      ...
                      />
                      <span
                        className={`absolute right-3 top-10 ${formik.values.newPassword ? 'cursor-pointer' : 'pointer-events-none opacity-50'
                          }`}
                        onClick={() => {
                          if (!formik.values.newPassword) return;
                    ...
                    <div className="flex flex-col mb-3 relative">
                      ...
                      />
                      <span
                        className={`absolute right-3 top-10 ${formik.values.confirmNewPassword.trim() ? 'cursor-pointer' : 'pointer-events-none opacity-50'
                          }`}
                        onClick={() => {
                          if (!formik.values.confirmNewPassword.trim()) return;
